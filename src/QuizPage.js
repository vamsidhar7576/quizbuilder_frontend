...
const QuizPage = () => {
  ...
  useEffect(() => {
    console.log("Selected Marks Use effect:", totalMarks);
  }, [totalMarks]);

  const fetchQuizList = async () => {
    try {
      const response = await axios.get("http://localhost:8090/api/quizzes");
  ...
  const fetchQuizQuest = async (quizId) => {
    try {
      const response = await axios.get(
        `http://localhost:8090/api/quizzes/getQuizQuestById/${quizId}`
      );
    ...
    try {
      ...
      for (const userAnswer of selectedOptions) {
        ...
        const response = await axios.post(
          "http://localhost:8090/api/user_answers",
          userAnswers
        );
